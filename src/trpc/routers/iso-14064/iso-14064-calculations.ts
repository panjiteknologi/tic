...
import { Iso14064AICalculator } from '@/lib/iso-14064-ai-calculator';
...
async function updateProjectSummary(projectId: string) {
  ...
  if (existingSummary.length > 0) {
    ...
    // Create new summary
    await db.insert(iso14064ProjectSummaries).values({
      projectId,
      scope1Total: scope1Total.toString(),
      scope2Total: scope2Total.toString(),
      scope3Total: scope3Total.toString(),
...
export const iso14064CalculationsRouter = createTRPCRouter({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const calculationResult = await Iso14064AICalculator.calculate({
        ...
        console.error('Error creating ISO 14064 calculation:', error);
        throw new TRPCError({
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const calculation = await db
        .select()
        .from(iso14064Calculations)
        .where(eq(iso14064Calculations.id, input.id))
        .limit(1);

      if (calculation.length === 0) {
        throw new TRPCError({
          code: 'NOT_FOUND',
          message: 'ISO 14064 calculation not found',
        });
      }
      ...
      const project = await db
        .select()
        .from(iso14064Projects)
        .where(eq(iso14064Projects.id, calculation[0].projectId))
        .limit(1);
  ...
  getByProjectId: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const calculations = await db
        .select()
        .from(iso14064Calculations)
        .where(eq(iso14064Calculations.projectId, input.projectId))
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const project = await db
          .select()
          .from(iso14064Projects)
          .where(eq(iso14064Projects.id, existingCalculation[0].projectId))
          .limit(1);
        ...
        if (needsRecalculation) {
          ...
          calculationResult = await Iso14064AICalculator.calculate({
        ...
        if (needsRecalculation && calculationResult) {
          updateData.activityData = input.activityData || existingCalculation[0].activityData;
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const project = await db
          .select()
          .from(iso14064Projects)
          .where(eq(iso14064Projects.id, projectId))
          .limit(1);
