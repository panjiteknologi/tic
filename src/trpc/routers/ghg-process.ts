...
const bulkAddStepTigaGhgCalculationProcessSchema = z.object({
  carbonProjectId: z.string().uuid(),
  items: z
    .array(
      z.object({
...
const bulkUpdateStepTigaGhgCalculationProcessSchema = z.object({
  items: z
    .array(
      z.object({
        id: z.number(),
...
export const ghgProcessRouter = createTRPCRouter({
  // Bulk add step tiga ghg calculation processes
  bulkAdd: protectedProcedure
    .input(bulkAddStepTigaGhgCalculationProcessSchema)
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        // Bulk insert step tiga ghg calculation processes
        const newStepTigaGhgCalculationProcesses = await db
        ...
        return {
          success: true,
          stepTigaGhgCalculationProcesses: newStepTigaGhgCalculationProcesses,
          count: newStepTigaGhgCalculationProcesses.length,
        };
      } catch (error) {
        ...
        throw new TRPCError({
          ...
          message: "Failed to bulk create step tiga ghg calculation processes",
        });
      }
    }),

  // Bulk update step tiga ghg calculation processes
  bulkUpdate: protectedProcedure
    .input(bulkUpdateStepTigaGhgCalculationProcessSchema)
    .mutation(async ({ ctx, input }) => {
      try {
        const updatedItems = [];
        ...
        for (const item of input.items) {
          ...
          const process = await db
            ...
            .innerJoin(
              carbonProject,
              eq(stepTigaGhgCalculationProcess.carbonProjectId, carbonProject.id)
            )
            .where(eq(stepTigaGhgCalculationProcess.id, item.id))
            .limit(1);

          if (process.length === 0) {
            throw new TRPCError({
              code: "NOT_FOUND",
              message: `Step tiga ghg calculation process with id ${item.id} not found`,
            });
          }
        ...
        return {
          success: true,
          stepTigaGhgCalculationProcesses: updatedItems,
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to bulk update step tiga ghg calculation processes",
        });
      }
    }),
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        // Delete step tiga ghg calculation process
        await db
          .delete(stepTigaGhgCalculationProcess)
          .where(eq(stepTigaGhgCalculationProcess.id, input.id));
  ...
  getByCarbonProjectId: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const projectProcesses = await db
        ...
        .where(
          eq(
            stepTigaGhgCalculationProcess.carbonProjectId,
            input.carbonProjectId
          )
        )
        .orderBy(asc(stepTigaGhgCalculationProcess.keterangan));
