...
export const ghgOtherCaseRouter = createTRPCRouter({
  ...
  bulkAdd: protectedProcedure
    .input(bulkAddStepTigaOtherCaseSchema)
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        // Bulk insert step tiga other cases
        const newStepTigaOtherCases = await db
        ...
        return {
          success: true,
          stepTigaOtherCases: newStepTigaOtherCases,
          count: newStepTigaOtherCases.length,
        };
      } catch (error) {
  ...
  getByCarbonProjectId: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const projectOtherCases = await db
        ...
        .orderBy(asc(stepTigaOtherCase.keterangan));
