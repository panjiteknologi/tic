...
export function LoginForm({ className, ...props }: LoginFormProps) {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      const { data: result, error: authErr } = await authClient.signIn.email({
        email: data.email,
      ...
      if (authErr) {
        setErrorMsg(
          authErr.message || "Authentication failed. Please try again."
        );
      } else if (result) {
        router.refresh();
        router.push("/apps");
        ...
        <CardContent className="grid p-0 md:grid-cols-2">
          ...
          >
            <Image
              ...
              width={200}
              height={128}
              className="h-32 w-auto dark:brightness-[0.2] dark:grayscale"
              priority
            />
          </Link>
        </CardContent>
