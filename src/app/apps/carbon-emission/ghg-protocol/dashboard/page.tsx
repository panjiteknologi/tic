...
const OverviewTab = () => {
  ...
  const { data: projectsData, isLoading: projectsLoading } =
    trpc.ghgProtocolProjects.getByTenantId.useQuery(
      { tenantId },
      { enabled: !!tenantId }
    );
  ...
  if (projectsLoading) {
    return (
      <motion.section
        ...
        transition={{ duration: 0.3 }}
      >
        <h2 className="text-lg font-semibold">📊 Ringkasan GHG Protocol</h2>
  ...
  return (
    ...
    >
      ...
      <p className="text-muted-foreground">
        Informasi mengenai proyek dan emisi karbon berdasarkan standar GHG
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        <div className="rounded-lg border p-4 bg-muted/50">
          ...
          <p className="text-sm text-muted-foreground">Proyek GHG Protocol</p>
        </div>
        <div className="rounded-lg border p-4 bg-muted/50">
...
const ProjectsTab = () => {
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold">📋 Daftar Proyek</h2>
      <p className="text-muted-foreground">
        Semua proyek GHG Protocol yang tersedia.
      </p>
...
export default function GHGProtocolDashboardPage() {
  const { data: userProfile } = trpc.user.getUserProfile.useQuery();
  const tenantId = userProfile?.tenantId ?? '';
  ...
  const stats = [
    {
      ...
      value: `${totalProjects}`,
      icon: Factory,
      description: 'Proyek GHG Protocol'
    },
    {
      title: 'Proyek Aktif',
