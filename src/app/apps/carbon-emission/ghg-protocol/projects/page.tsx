...
import { GHGProtocolProjectDialogForm } from '@/components/ghg-protocol/ghg-protocol-project-dialog-form';
...
export default function GHGProtocolProjectsPage() {
  ...
  const { data, isLoading } = trpc.ghgProtocolProjects.getByTenantId.useQuery(
    { tenantId },
    { enabled: !!tenantId }
  );
  ...
  const createMutation = trpc.ghgProtocolProjects.create.useMutation({
    onSuccess: (data) => {
      utils.ghgProtocolProjects.getByTenantId.invalidate({ tenantId });
      setFormData({
        name: '',
        description: '',
        organizationName: '',
  ...
  const updateMutation = trpc.ghgProtocolProjects.update.useMutation({
    onSuccess: (data) => {
      utils.ghgProtocolProjects.getByTenantId.invalidate({ tenantId });
      setFormData({
        name: '',
  ...
  const deleteMutation = trpc.ghgProtocolProjects.delete.useMutation({
    onSuccess: () => {
      utils.ghgProtocolProjects.getByTenantId.invalidate({ tenantId });
  ...
  const handleEdit = (project: (typeof projects)[0]) => {
    setFormData({
      ...
      reportingPeriodStart:
        project.reportingPeriodStart instanceof Date
          ? project.reportingPeriodStart
          : new Date(project.reportingPeriodStart),
      reportingPeriodEnd:
        project.reportingPeriodEnd instanceof Date
      ...
      boundaryType: project.boundaryType as
        | 'operational'
        | 'financial'
        | 'other',
      standardVersion:
        project.standardVersion || 'GHG Protocol Corporate Standard'
    });
    setOpenDialog(true);
    setEditMode(true);
      ...
      <div className="space-y-6 mt-6">
        ...
        <div className="mt-8">
          {isLoading ? (
              ...
              {filteredProjects.map((project, index) => (
                ...
                >
                  <Card
                    ...
                    onClick={() =>
                      router.push(
                        `/apps/carbon-emission/ghg-protocol/projects/${project.id}`
                      )
                    }
                  >
                    <CardHeader>
            ...
            <NoData message="Belum ada proyek GHG Protocol yang tersedia." />
          )}
        </div>
      </div>
