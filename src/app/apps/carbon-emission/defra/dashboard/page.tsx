...
const OverviewTab = () => {
  ...
  const { data: projectsData, isLoading: projectsLoading } =
    trpc.defraProjects.getByTenantId.useQuery(
      { tenantId },
      { enabled: !!tenantId }
    );
  ...
  if (projectsLoading) {
    return (
      <motion.section
        ...
        transition={{ duration: 0.3 }}
      >
        <h2 className="text-lg font-semibold">📊 Ringkasan DEFRA</h2>
  ...
  return (
    ...
    >
      ...
      <p className="text-muted-foreground">
        Informasi mengenai proyek dan emisi karbon berdasarkan standar DEFRA.
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        <div className="rounded-lg border p-4 bg-muted/50">
          ...
          <p className="text-sm text-muted-foreground">Proyek DEFRA</p>
        </div>
        <div className="rounded-lg border p-4 bg-muted/50">
            ...
            {projects.slice(0, 5).map((project) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="text-sm font-medium">{project.defraYear}</p>
...
const ProjectsTab = () => {
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold">📋 Daftar Proyek</h2>
      <p className="text-muted-foreground">Semua proyek DEFRA yang tersedia.</p>
      ...
      {projects.length > 0 ? (
            ...
            >
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground mt-2">
                  <span>Tahun: {project.defraYear}</span>
        ...
        <div className="text-center py-8 text-muted-foreground">
          Belum ada proyek DEFRA yang tersedia.
        </div>
      )}
    </motion.section>
  );
...
const TrendsTab = () => {
  ...
  const { data: projectsData, isLoading: trendsLoading } =
    trpc.defraProjects.getByTenantId.useQuery(
      { tenantId },
      { enabled: !!tenantId }
    );
  ...
  if (trendsLoading) {
    return (
      <motion.section
        ...
        transition={{ duration: 0.3 }}
      >
        <h2 className="text-lg font-semibold">📈 Tren Proyek</h2>
  ...
  const chartData = projects.reduce((acc: any[], project) => {
    const existingYear = acc.find((item) => item.year === project.defraYear);
    if (existingYear) {
      existingYear.projectCount += 1;
    } else {
      acc.push({
        year: project.defraYear,
        projectCount: 1
      });
    }
    return acc;
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold">📈 Tren Proyek</h2>
      <p className="text-muted-foreground">
        Tren proyek DEFRA dari tahun ke tahun.
      </p>
      ...
      {chartData.length > 0 && (
        <div className="rounded-lg border bg-white p-4 mt-2">
          <LineCharts
            ...
            description="Jumlah proyek per tahun"
          />
        </div>
      )}
          ...
          <p className="text-lg font-bold">
            {chartData.length > 0
              ? `${Math.min(
                ...
                )} - ${Math.max(...chartData.map((d) => parseInt(d.year)))}`
              : '-'}
          </p>
        </div>
        <div className="rounded-lg border p-4 bg-muted/50">
          <h3 className="font-medium text-green-800">Total Proyek</h3>
          <p className="text-lg font-bold">{projects.length}</p>
        </div>
        <div className="rounded-lg border p-4 bg-muted/50">
          <h3 className="font-medium text-purple-800">Tahun Aktif</h3>
          <p className="text-lg font-bold">{chartData.length} tahun</p>
        </div>
      </div>
    </motion.section>
  );
...
export default function DEFRADashboardPage() {
  const { data: userProfile } = trpc.user.getUserProfile.useQuery();
  const tenantId = userProfile?.tenantId ?? '';

  const { data: projectsData } = trpc.defraProjects.getByTenantId.useQuery(
    { tenantId },
    { enabled: !!tenantId }
  );
  ...
  const stats = [
    ...
    {
      title: 'Tahun Aktif',
      value: `${new Set(projects.map((p) => p.defraYear)).size}`,
      icon: TrendingUp,
